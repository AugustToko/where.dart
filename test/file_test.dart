  ...
  group('where()', () {
    test('should return the path of the `executable.cmd` file on Windows', () async {
      try {
        ...
        else expect(executable, allOf(isInstanceOf<String>(), endsWith(r'\test\fixtures\executable.cmd')));
      }

      on Exception catch (err) {
        if (Finder.isWindows) fail(err.toString());
        else expect(err, isInstanceOf<FinderException>());
      }
    });
    ...
    test('should return all the paths of the `executable.cmd` file on Windows', () async {
      try {
        ...
        if (!Finder.isWindows) fail('Exception not thrown');
          ...
          expect(executables.first, allOf(isInstanceOf<String>(), endsWith(r'\test\fixtures\executable.cmd')));
        }
      }
    ...
    test('should return the path of the `executable.sh` file on POSIX', () async {
      try {
        ...
        if (Finder.isWindows) fail('Exception not thrown');
        else expect(executable, allOf(isInstanceOf<String>(), endsWith('/test/fixtures/executable.sh')));
      }
    ...
    test('should return all the paths of the `executable.sh` file on POSIX', () async {
      try {
        ...
        if (Finder.isWindows) fail('Exception not thrown');
          ...
          expect(executables, allOf(isList, hasLength(1)));
          expect(executables.first, allOf(isInstanceOf<String>(), endsWith('/test/fixtures/executable.sh')));
        }
      }
